...
async function downloadResultCard() {
  ...
  ctx.fillText(state.petName, 125, 360);
  ctx.fillStyle = result.color;
  ctx.font = '700 98px "PingFang SC", Arial';
  ctx.fillText(result.name, 125, 475);
  try {
  ...
  ctx.fillText(result.tagline, 125, 545);
...
async function copyShareText() {
  const text = `我家${state.petName}的 Mini Mates 宠格是「${state.result.name}」\n${state.result.tagline}\n你家毛孩子会是哪一种？`;
  try {
